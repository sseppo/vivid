> **Status:** historical — family F6; superseded by `docs/context/sources/02-business-logic-constitution.md`. Kept for provenance; not current. Source: `Visualisatium/planning/handover-to-new-sessions/valiold/old/02_Business_Logic_Constitution_BusinessLogicEconomics_v6.md`. 

# BUSINESS LOGIC CONSTITUTION (v6.0)
> **Status:** GOVERNING DOCUMENT
> **Source:** User Manual Construction + Specific Corrections
> **Currency Peg:** 1 Credit = €0.10 (Ten Cents).
...
The purpose is that only Content Creators make the requests that require man hours / manual work. If the work is solely AI based automated workflow, it belongs to default Content Creator.
...
*   **Split Payment:** If customer pays partly with credits, the invoice is replaced with two new ones, one of which indicates already paid part with credits, and another one to be paid with transaction. Also they are marked accordingly to send correct data for accounting purposes. In dashboard, both invoices are then shown under same product purchase.

**The "First Hour" Rule (Deposits):**
All invoiced products include manual work as man hours. Thus to start the process, first a minimum amount is charged. This includes the creation of the actual offer or invoice. In the later created invoice (by the offer made) the already purchased hour is taken into concern. This means that also invoiced products start with purchase by credits or transaction.
...
*   **Standard Option:** 1 Manhour (**€75.00** / 750 Credits).
*   **Small Option:** 0.5 Manhour (**€37.50** / 375 Credits). Available for smaller tasks where offer preparation is quick.

Work starts only after the invoice has been paid. If it needs to be broken to smaller invoices, that can be done from the admin dashboard and customer can pay them one by one. However, same applies there: work is done in periods defined by those invoices, and only continues when next invoice is paid. All invoiced and work-requiring man hours go to job queue, from where they may be picked up for work.
...
Customer has option to ask for offer before any payment is made, or customer may purchase 1 hour (or 0.5 hour) of work in order for CCA to make the offer (which is then deducted from the final total). Product is in state of offer until it has been accepted or rejected. Customer may make counter offer, and thus offer can go back-and forth until it is rejected or accepted. If there is no response for long time, it will go to dormant state.
...
The system has one default Content Creator, set when developing the site. All automated workflows made by AI automation (not requiring manual work) go to this default Content Creator. 
...
Before a human Admin sees a dispute, use the Default Creator (AI) to analyze:
*  The Prompt (Request).
...
For longer video sets (Sagas), we utilize **Shotstack API** to stitch generated clips together, as native AI models (Runway/Luma) typically generate short clips (5s). This ensures we can deliver the "Product Promise" of longer videos.
...
## 14. SUPPLY CHAIN LOGIC (Virtual Warehouse)
...
## 15. AUTOMATION & SUBSCRIPTION LOGIC (The "Creation Service" Protocol)
...
---
## 19. THE ABANDONMENT & RESTORATION PROTOCOL ("Phoenix")
...
## 21. SPECS: THE SMART LEDGER (Economic Engine)
...
## 22. THE "BOOK" PRICING MODEL (By Arc, Not Chapter)
