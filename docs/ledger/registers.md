...
`YYYY-MM-DD-librarian-sNN-<topic>.md`. Until pass 4 part 2 copies them into `docs/handoff/archive/`
under the same names, they sit in `C:\api\vividvis\valmistelu\from-librarian\`:
...
|---|---|---|---|---|
| `D-n` | decisions that bind the project beyond the reorganisation | `docs/ledger/decisions.md` | D-12 | 2026-09-19, against that file |
| `L-n` | learnings | `docs/ledger/learnings.md` | L-13 | 2026-09-19, against that file |
| `R-n` | owner rulings on Librarian questions | this file | R-36 | 2026-09-19, rows below |
| `ND-n` | needs-decision items | this file | ND-14 | 2026-09-19, rows below; ND-1…ND-8 against s1, plan §Needs decision, s5 |
...
| channel ids | `<x>2<y>-n` per channel file | `docs/channels/` | none yet | prefixes reserved below |
...
| R-6 | 2026-09-17 | The two empty `.git` shells in the source folders are left untouched and listed in the deletion proposal at close | **open** — deletion proposal not yet written (pass 4 close) | s1 ruling 6 |
...
| R-14 | 2026-09-18 | Q4: project rules are a **merge** — `project_rules.md` as base + §4 from `project_rules_someNewVersion.md`; both originals archived | in force → D-6, ADR 0004; merge file is pass 4 part 2 | s3 ruling 3 |
...
| R-31 | 2026-09-19 | ND-8 (b): the legacy provenance note is `legacy/README.md`; `legacy/visualisatium-web/README.md` stays a byte copy | in force; file is pass 4 part 2 | s5 addendum |
...
| ND-9 | Q7 | `04_` (tech stack) and `05_` (schema) masters were not in the code-era input set `vivid/.chat_attachments/start/` — deliberate? If not, the code session started without them | **open** | owner, then Architect |
| ND-10 | Q9 | `docs/brainstorm/api-visualisatium/prompts/muistiin'.txt` lists open technical questions (splitting user and product databases, where passwords live, delivering video to a customer's own YouTube channel) that nothing else answers — transcode and answer? | **open** | Architect |
| ND-11 | Q10 | Restate the credit peg once, authoritatively (X-1, X-2)? A product decision | **open** | owner |
| ND-12 | Q11 | `Products.xlsx` covers Image and Video only; the other `Tuotteet-*` sheets are not in it — is the catalogue those two, or is `Products.xlsx` partial? | **open** | owner |
| ND-13 | Q12 | Rotate the Anthropic and OpenAI keys held in `valmistelu\legacy-env\`? Deferred to the end of the reorganisation, risk accepted (R-20) | **open, deferred** | owner |
| ND-14 | — | Creator Gem v3.3 and Guidance Gem v3.4 differ from their Takeout copies by 70 and 38 non-whitespace characters; which text is authoritative? | **open** | Architect (R-28) |

## X — contradictions (from `INVENTORY.md` §Contradictions, 2026-09-17)

| id | alias | contradiction | status |
|---|---|---|---|
| X-1 | C1 | Credit peg: 1 credit = €1.00 (constitution v2.5) vs €0.10 (v6); the later line appears to be v6 | **open** — ND-11 |
| X-2 | C2 | Three pricing tables give three credit values for Image (1 cr/€5 · 1 cr/€1 · 10 cr/€1); the newest is `PricingTable_v2` (2026-02-04) | **open** — ND-11 |
...
|---|---|---|
| `l2a` | `docs/channels/librarian-to-architect.md` | reserved; first entry in pass 4 part 2 |
