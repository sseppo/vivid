> **Status:** current edition (F12, newest). A fuller earlier edition is at `docs/context/sources/07-features-ux-fullest-edition.md`; later revisions lost content to Gem context limits. Reconciliation is Architect work. Source: `vivid/.chat_attachments/start/07_Features_UX_Master_v5.md`. 
...
**Logic:**
    * **Goal:** A gauge on the dashboard (`Preservation_Gauge` module). Encourage users to use the "Partner Digitization" service.
...
### C. PRICING MODEL TWEAK: "THE LEGACY SUBSCRIPTION"
Current pricing is transactional. We need recurring revenue.
* **The Offer:** "Archive Tier" (e.g. €9/month).
* **Benefits:**
...
6.  **Dashboard View:** `Story Context Panel` shows these answers to the CCA while they edit.

### B. The "Digital Person" Subscription
**Objective:** Turn one-off "Digital Person" sales into recurring revenue (MRR).
**Economic Model:**
* **Product:** "Digital Person Hosting"
...
* **Feature:** If credits run out, the Person goes "Dormant" (Storage only, no interactive API access).
...
#### The "Collection Canvas" (User Dashboard)
    ...
    * **Right:** The Outputs.
    * **Why:** It looks like a "Studio" or like a game rather than a "Shopping Cart".
	* **Status Indicators:**
...
* **Context-Aware Chat:** Chat widgets about orders are linked to `Order_ID` or `Ticket_ID`. 
...
* **Multi-Party Support:** Chat supports for example `User + Admin + Partner` visibility if Physical Goods are involved.
...
* **Unlock:** The Clean Version is only released to the "Files" tab once the status moves to `Completed`.
* **Review Period:** during Review Period, the user only sees the Watermarked preview. To get the clean file early, they must trigger the "Irreversible Action" (Acceptance)
